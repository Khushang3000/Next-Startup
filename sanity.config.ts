...
// it basically gives me the list of all startups, and we just need to define the schema.
  ...
  plugins: [
    ...
    //we'll do fetching of real data in the next commit
  
    //in order to do that we have to use something like groqq queries. it is sanity's own open source querying language
    //see that on the sanity docs.
    //now we'll visually see how it works, go to localhost:3000/studio, from there click on the vision tab.
    //you'll see the query and the params section, now in the query type *, to get all the results obv
    //you'll get everything related to sanity project from members to grants, startups, authors and everything

    //first property will be the user that created the project, scroll down to see more properties and you'll even see the authors and the post
    //*[0], just give the first property(user that created the project)

    //if we want to get all the startups then
    //*[_type == "startup"]
    //now if you see it closely, it doesn't provide the author information, rather it just gives the reference to that document.
    //you'll also see some more properties in that startup, which are (not unnecessary but we didn't create them, sanity did.)
    //so how do we query more specifically?
    //*[_type == "startup" && defined(slug.current)]{
    // //in here we define exactly what we need.
    // _id,
    //  title,
    //  slug, 
    // _createdAt,
    ...
    //now we obv don't want to write queries in the vision tab for sanity studio, rather go to lib/ create queries.ts
  ],
})
